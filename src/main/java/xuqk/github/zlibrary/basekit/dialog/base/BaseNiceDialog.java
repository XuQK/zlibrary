...
public abstract class BaseNiceDialog extends DialogFragment {
    ...
    private void initParams() {
        ...
        if (window != null) {
            ...
            //设置dialog宽度
            if (width == 0) {
                lp.width = Kits.Device.getScreenWidth(getActivity()) - 2 * Kits.Dimens.dpToPxInt(getActivity(), margin);
            } else if (width == -1) {
                lp.width = WindowManager.LayoutParams.WRAP_CONTENT;
            } else {
                lp.width = Kits.Dimens.dpToPxInt(getActivity(), width);
            }
            ...
            if (height == 0) {
                ...
                lp.height = Kits.Dimens.dpToPxInt(getActivity(), height);
            }
